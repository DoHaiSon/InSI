...
for Monte_i = 1:Monte
    ...
    MCRB_SB_trace = zeros(1,length(SIGMA));
    MCRB_OP_trace = zeros(1,length(SIGMA));
    ...
    for k = 1 : length(SIGMA)
      ...
      %% Partial derivative of C_y w.r.t. theta_i
        ...
        GMCRB_OP     = pinv(A_OP) * J_OP * pinv(A_OP);
        
        MCRB_SB_trace(1,k) = abs(trace(GMCRB_SB(1:L_hpt,1:L_hpt) ) ) ;
        MCRB_OP_trace(1,k) = abs(trace(GMCRB_OP(1:L_hpt,1:L_hpt) ) ) ;
    end
    
    Err_f = [Err_f; MCRB_SB_trace];
end
