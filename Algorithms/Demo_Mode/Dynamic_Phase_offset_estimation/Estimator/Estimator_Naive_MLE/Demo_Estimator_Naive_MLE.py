...
def distance_t(a, b, offset):
    diff = np.expand_dims(np.angle(a) - np.angle(b), axis=1)
    diff = np.where(diff < np.pi, diff, 2 * np.pi - diff)
    diff = np.where(diff > -np.pi, diff, 2 * np.pi + diff)
    abs_diff = np.abs(diff - offset)
    return np.squeeze(diff), abs_diff #+ sigmoid(np.abs(np.abs(a) - np.abs(b)) - 1)


def naive_ml_dynamic_phase_offset(Y, alphabet):
    N, K = Y.shape
    alphabet_size = alphabet.shape[0]
    theta_est = []
    for m in range(N):
        y = Y[m]
        diff = np.zeros(alphabet_size)
        prevs = []
        path = []
        accum_offset = np.zeros(alphabet_size)
        for i in range(K):
            diff, cur_offset = distance_t(y[i], alphabet, diff)
            accum_offset = cur_offset + accum_offset
            prev = np.argmin(accum_offset, axis=1)
            accum_offset = np.min(accum_offset, axis=1)
            cur_offset = cur_offset[np.arange(cur_offset.shape[0]), prev]
            prevs.append(prev)
            path.append(diff)

            # trace back
        est = np.zeros(K)
        min_i = np.argmin(accum_offset)
...
if __name__ == "__main__":
    ...
    MSE_sig   = np.mean(np.square(theta_est - theta.T))
