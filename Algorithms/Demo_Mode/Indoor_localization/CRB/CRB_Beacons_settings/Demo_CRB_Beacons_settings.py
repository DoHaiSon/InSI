...
if __name__ == "__main__":
    ...
    data = loadmat(os.path.join(current_path, 'Params', 'RSS_8_beacons_indoor.mat'))
    index = [B1, B2, B3, B4, B5, B6, B7, B8]

    data['x'] = data['x'][:, index]
    F_BCRB = FINE_BCRB(data['theta'], data['x'], **FINE_settings)


    print(np.diag(F_BCRB))
    ...
    with open(file_path, 'a') as result:
        result.write('\n')
        result.write(str(np.count_nonzero(index)))
        result.write('\n')
        result.write(str(np.diag(F_BCRB)))