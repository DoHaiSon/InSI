...
for Nr_ring_index=1:Nr_ring
    for Nr_UCA_index=1:Nr_UCA
        Br_fading = spec_chan_derive_fading_Hex(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_UCA_index,Nr_ring_index,Nr_UCA,L,M,Nt);
        dev_h_fading=[dev_h_fading; transpose(Br_fading)];

        Br_delay = spec_chan_derive_delay_Hex(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_UCA_index,Nr_ring_index,Nr_UCA,L,M,Nt);
        dev_h_delay=[dev_h_delay; transpose(Br_delay)];

        Br_angle_Phi = spec_chan_derive_angle_Phi_Hex(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_UCA_index,Nr_ring_index,Nr_UCA,L,M,Nt);
        dev_h_angle_Phi=[dev_h_angle_Phi; transpose(Br_angle_Phi)];

        Br_angle_Theta = spec_chan_derive_angle_Theta_Hex(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_UCA_index,Nr_ring_index,Nr_UCA,L,M,Nt);
...
for snr_i = 1 : length(SNR)
    ...
    %Iop_spec = DD' * Iop * DD;
    Iop_spec = ((-1)/(sigmav2)^2)*G*G'* X_nga'*sigmav2*eye(Nr_ring*Nr_UCA*K)*X_nga*G*G';
