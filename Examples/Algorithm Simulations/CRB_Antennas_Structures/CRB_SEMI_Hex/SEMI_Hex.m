...
Nr_ring = 8;         % number of receive antennas of UCA
L       = 4;         % channel order
...
d_ring_nor=0.5;
d_UCA_nor=0.5;
R_nor=0.5*d_UCA_nor/sin(pi/Nr_UCA)+d_ring_nor*(0:1:(Nr_ring-1));
...
for Nr_ring_index=1:Nr_ring
    for Nr_UCA_index=1:Nr_UCA
        Br_fading = SEMI_spec_chan_derive_fading_Hex(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_UCA_index,Nr_ring_index,Nr_UCA,L,M,Nt);
        dev_h_fading=[dev_h_fading; transpose(Br_fading)];

        Br_delay = SEMI_spec_chan_derive_delay_Hex(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_UCA_index,Nr_ring_index,Nr_UCA,L,M,Nt);
        dev_h_delay=[dev_h_delay; transpose(Br_delay)];

        Br_angle_Phi = SEMI_spec_chan_derive_angle_Phi_Hex(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_UCA_index,Nr_ring_index,Nr_UCA,L,M,Nt);
        dev_h_angle_Phi=[dev_h_angle_Phi; transpose(Br_angle_Phi)];

        Br_angle_Theta = SEMI_spec_chan_derive_angle_Theta_Hex(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_UCA_index,Nr_ring_index,Nr_UCA,L,M,Nt);
...
[H, h_true] = gen_chan_specular(fading,delay,DOA_Phi,DOA_Theta,R_nor,Nr_ring,Nr_UCA,L,Nt);

%LAMBDA
LAMBDA  = [];
for jj = 1 : Nt
    lambda_j =[];
    for r = 1 : Nr_ring*Nr_UCA
...
%============================================
%Only Pilot    
    X_nga=kron(eye(Nr_ring*Nr_UCA),X);
%============================================
%Only Pilot Normal
...
%============================================
%SemiBlind
    Cyy      = sigmax2 * LAMBDA * LAMBDA'  + sigmav2 * eye(K*Nr_ring*Nr_UCA);
    Cyy_inv  = pinv(Cyy);
    ...
    I_D = triu(repmat(I_D, Nr_ring*Nr_UCA*Nt, Nr_ring*Nr_UCA*Nt));
%============================================
%Semiblind Normal
