function varargout = Algo_SideInfor_Menu(varargin)
% ALGO_SIDEINFOR_MENU MATLAB code for Algo_SideInfor_Menu.fig
%      ALGO_SIDEINFOR_MENU, by itself, creates a new ALGO_SIDEINFOR_MENU or raises the existing
%      singleton*.
%
%      H = ALGO_SIDEINFOR_MENU returns the handle to a new ALGO_SIDEINFOR_MENU or the handle to
%      the existing singleton*.
%
%      ALGO_SIDEINFOR_MENU('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ALGO_SIDEINFOR_MENU.M with the given input arguments.
%
%      ALGO_SIDEINFOR_MENU('Property','Value',...) creates a new ALGO_SIDEINFOR_MENU or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Algo_SideInfor_Menu_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Algo_SideInfor_Menu_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Algo_SideInfor_Menu

% Last Modified by GUIDE v2.5 29-Jul-2023 17:36:14
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Algo_SideInfor_Menu_OpeningFcn, ...
                   'gui_OutputFcn',  @Algo_SideInfor_Menu_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Algo_SideInfor_Menu is made visible.
function Algo_SideInfor_Menu_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user method (see GUIDATA)
% varargin   command line arguments to Algo_SideInfor_Menu (see VARARGIN)

    global main_path;
    jFrame=get(handle(handles.InSI_A_SI), 'javaframe');
    ...
    % Choose default command line output for Algo_SideInfor_Menu
    handles.output = hObject;
    
    % Update handles structure
    guidata(hObject, handles);
    
    % UIWAIT makes Algo_SideInfor_Menu wait for user response (see UIRESUME)
    % uiwait(handles.InSI_A_SI);


% --- Outputs from this function are returned to the command line.
function varargout = Algo_SideInfor_Menu_OutputFcn(hObject, eventdata, handles) 
...
% handles    empty - handles not created until after all CreateFcns called
    ...
    methods = load_methods(default, 'Algo_Mode', 'Side-information');
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    vers = load_versions('Algo_Mode', 'Side-information', default, methods{method});
...
%        contents{get(hObject,'Value')} returns selected item from version
    ...
    load_params(hObject, eventdata, handles, 'Algo_Mode', 'Side-information', vers{ver});
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    load_funcs(hObject, eventdata, handles, 'Algo_Mode', 'Side-information', algo);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    load_reactive(hObject, eventdata, handles, 'Algo_Mode', 'Side-information', algo);
