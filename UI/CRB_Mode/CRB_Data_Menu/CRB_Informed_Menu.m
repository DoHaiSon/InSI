function varargout = CRB_Informed_Menu(varargin)
% CRB_INFORMED_MENU MATLAB code for CRB_Informed_Menu.fig
%      CRB_INFORMED_MENU, by itself, creates a new CRB_INFORMED_MENU or raises the existing
%      singleton*.
%
%      H = CRB_INFORMED_MENU returns the handle to a new CRB_INFORMED_MENU or the handle to
%      the existing singleton*.
%
%      CRB_INFORMED_MENU('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CRB_INFORMED_MENU.M with the given input arguments.
%
%      CRB_INFORMED_MENU('Property','Value',...) creates a new CRB_INFORMED_MENU or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CRB_Informed_Menu_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CRB_Informed_Menu_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CRB_Informed_Menu

% Last Modified by GUIDE v2.5 28-Jul-2023 16:11:17
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @CRB_Informed_Menu_OpeningFcn, ...
                   'gui_OutputFcn',  @CRB_Informed_Menu_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CRB_Informed_Menu is made visible.
function CRB_Informed_Menu_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user method (see GUIDATA)
% varargin   command line arguments to CRB_Informed_Menu (see VARARGIN)

    global main_path;
    jFrame=get(handle(handles.InSI_C_I), 'javaframe');
    ...
    % Choose default command line output for CRB_Informed_Menu
    handles.output = hObject;
    
    % Update handles structure
    guidata(hObject, handles);
    
    % UIWAIT makes CRB_Informed_Menu wait for user response (see UIRESUME)
    % uiwait(handles.InSI_C_I);


% --- Outputs from this function are returned to the command line.
function varargout = CRB_Informed_Menu_OutputFcn(hObject, eventdata, handles) 
...
% hObject    handle to methods (see GCBO) Informed
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
    ...
    methods = load_methods(default, 'CRB_Mode', 'Informed');
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    algo = strcat('I_', methods{method});
    load_params(hObject, eventdata, handles, 'CRB_Mode', 'Informed', algo);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    algo = strcat('I_', methods{method});
    load_funcs(hObject, eventdata, handles, 'CRB_Mode', 'Informed', algo);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    algo = strcat('I_', methods{method});
    load_reactive(hObject, eventdata, handles, 'CRB_Mode', 'Informed', algo);
